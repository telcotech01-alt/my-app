...
app = Flask(__name__)
app.secret_key = "change-this-to-any-random-string-123"  # প্রোডাকশনে বদলাবে
DB = "billing.db"
...
def dashboard():
    ...
    history = conn.execute('''SELECT h.*, r.username FROM history h
                              JOIN reseller r ON h.reseller_id = r.id 
...
def add_reseller():
    ...
    try:
        ...
        conn.commit()
        flash('রিসেলার সফলভাবে অ্যাড হয়েছে')
...
def add_balance():
    ...
    conn.execute("INSERT INTO history (reseller_id, type, amount, note, date) VALUES (?, ?, ?, ?, ?)",
                 (reseller_id, 'Credit', amount, 'Admin Balance Add', 
                  datetime.now().strftime('%Y-%m-%d %H:%M')))
    conn.commit()
    conn.close()
    flash('ব্যালেন্স সফলভাবে যোগ হয়েছে')
    return redirect(url_for('dashboard'))
